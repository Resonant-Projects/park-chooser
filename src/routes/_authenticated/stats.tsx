...
import { Loader2, MapPin, TrendingUp, Trophy } from "lucide-react";
...
function StatsPage() {
	...
	const totalVisits =
		userParks?.reduce((sum: number, up: { visitCount: number }) => sum + up.visitCount, 0) ?? 0;
	const parksWithVisits =
		userParks?.filter((up: { visitCount: number }) => up.visitCount > 0) ?? [];
	...
	return (
		<main className="flex-1 flex flex-col items-stretch justify-start w-full">
			<div className="mb-6">
				...
				<p className="text-[var(--color-mist)]">Track your park adventures</p>
			</div>
