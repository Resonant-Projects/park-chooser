...
function AppPage() {
  ...
  // Travel time state
  const [travelTime, setTravelTime] = useState<{ durationText: string; distanceText: string } | null>(null)
  ...
  const expiryMessage = status === 'canceled' && periodEnd && isPremium
  ...
  const handlePickPark = async () => {
    ...
    try {
      ...
      if (message.includes('DAILY_PICK_LIMIT_EXCEEDED')) {
        ...
        setError('Add some parks to your list first! Visit the Manage page to get started.')
      } else {
        setError(message)
      }
    } finally {
    ...
    <main className="flex-1 flex flex-col items-center justify-center gap-6">
      ...
      {todaysPick ? (
        <div className="park-card-main animate-card-appear max-w-md w-full">
          {/* Photo Carousel */}
          {photos.length > 0 && (
            <div className={`carousel-container ${!hasMultiplePhotos ? 'single-photo' : ''}`}>
              <div
                ref={carouselRef}
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              {isTravelTimeLoading ? (
                ...
                <div className="travel-badge">
                  ...
                  <span>{travelTime.durationText} • {travelTime.distanceText}</span>
                </div>
              ) : (
                <button
