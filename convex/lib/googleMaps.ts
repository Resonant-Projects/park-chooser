...
/**
 * Generate a photo URL from a Legacy Places API photo reference.
...
/**
 * Search for nearby parks using Legacy Places API with pagination.
 * Returns up to 60 parks (3 pages of 20) within the specified radius.
 *
 * Note: The Legacy API only supports one type per request, so we search for "park" only.
 ...
 * NOTE: This API is limited to 20 results with no pagination.
