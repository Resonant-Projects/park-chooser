...
import { calculateDistanceMiles, searchNearbyParksWithPagination } from "../lib/googleMaps";
...
export interface NearbyParkResult {
	...
	name: string;
	address?: string;
	photoRef?: string; // First photo reference (use /api/photo proxy to fetch)
	photoRefs?: string[]; // All photo references (use /api/photo proxy to fetch)
...
export const searchNearbyParks = action({
	...
	handler: async (ctx, args): Promise<NearbyParkResult[]> => {
		...
		// Call Google Places API (Legacy) with pagination for up to 60 results
		const nearbyParks = await searchNearbyParksWithPagination(
			args.lat,
			args.lng,
			radiusMeters,
			apiKey
		...
		for (const park of nearbyParks) {
			...
			// Return photo references (not URLs) - frontend will use proxy endpoint
			const photoRefs = park.photoRefs.slice(0, 5);
