...
import {
  searchNearbyParksWithPagination,
...
export const searchNearbyParks = action({
  ...
  handler: async (ctx, args): Promise<NearbyParkResult[]> => {
    ...
    const nearbyParks = await searchNearbyParksWithPagination(args.lat, args.lng, radiusMeters, apiKey);
    ...
    for (const park of nearbyParks) {
      ...
      // Generate photo URLs from Legacy API photo references
      let photoUrl: string | undefined;
      let photoUrls: string[] = [];

      if (park.photoRefs.length > 0) {
        photoUrls = park.photoRefs.slice(0, 5).map((ref) => getLegacyPhotoUrl(ref, apiKey, 800));
