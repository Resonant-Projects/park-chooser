...
export const getUnusedDiscountCodes = internalQuery({
	...
	handler: async (ctx, args) => {
		...
		// Filter already ensures discountCode exists
		return rewards
			.filter((r): r is typeof r & { discountCode: string } => r.discountCode !== undefined)
			.map((r) => ({
				code: r.discountCode,
